...
void nextUnitProcess(void) {
	// check current process index and match the time spot
...
void readyQtoCPU(void) {
	...
	qsort(tempArray, tempArrayIndex, sizeof(process *), compareByArrival);
...
void cpuToio(void) {
	...
	for(i = 0; i < NUMBER_OF_PROCESSORS; i++) {
		...
		if(cpus[i] != NULL) {
			...
			if(cpus[i]->bursts[cpus[i]->currentBurst].step ==  cpus[i]->bursts[cpus[i]->currentBurst].length ? 1:0) {
				(cpus[i]->currentBurst)++;
...
int main() {
	...
	float avgWaiting, avgTurnAround, avgUtil;
	clockTime = 0;
	processIndex = 0;
	...
	*/
	...
	avgWaiting = totalWaiting / (float)numberOfProcesses;
	printf("%d ",totalWaiting);
