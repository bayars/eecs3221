...
int main ( int argc, char *argv[] ){
    ...
    for( int k = 0;k < argc -1;k++){
        ...
        int isWorked = pthread_create(&tid[k],&attr[k],threadFunc,argv[k+1]);   // Create thread with loop. Move argument with argv[k+1] 
        if( isWorked ){     // Check the thread created successfully
            printf("ERROR %d", isWorked);
            exit(-1);  
        }
        pthread_join(tid[k],NULL);  // Join thread with thread id
        printf("%s\n",result);      // print the calculated value in result variable
    }
    printf("MINIMUM=%lf MAXIMUM=%lf \n",totalmin,totalmax);     // Loop finished. print total max/min static variables
